			...
			if (s->pid!=s->tid) { // maintain a thread list for each process
				...
				if (p) {
					...
					if (p->threads) {
						...
						// printf("pid_cb %d %d %d\r\n", s->read_bytes, s->write_bytes,ret);
					}
				}
			}
			ret = arr_add(a,s);
			// printf("pid_cb %d %d %d %x\r\n", s->read_bytes, s->write_bytes,ret, a);
		}
	}
}
...
inline struct xxxid_stats_arr *fetch_data(filter_callback filter) {
	...
	// printf("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxa is %x\r\n", a);
	if (!a)
		return NULL;

	pidgen_cb((pg_cb)pid_cb,a,filter);
	// printf("fetch_data %d %d \r\n", a->read_bytes, a->write_bytes);
	// printf("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxa2 is %x\r\n", a);
	int i;
	for (i=0;i<a->length;i++) {
		// printf("fetch_data %d %d %x\r\n", a->arr[i]->read_bytes, a->arr[i]->write_bytes, a);
	}
	return a;
}
